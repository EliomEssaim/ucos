...
*/
//三层要求
void  OSTimeDly (INT16U ticks)//主动休息吧自己挂起所以肯定是正在运行的任务 ticks 被定时器中断管理
{
    INT8U      y;
    ...
    if (ticks > 0) {                             /* 0 means no delay!                                  */
        ...
        if (OSRdyTbl[y] == 0) {  
            OSRdyGrp &= ~OSTCBCur->OSTCBBitY;//取消就绪 清零
        }
        OSTCBCur->OSTCBDly = ticks;              /* Load ticks in TCB                                  *///直接是用正在运行任务的的指针 修改tcb
        OS_EXIT_CRITICAL();
        OS_Sched();                              /* Find next task to run!                             *///改变最高优先级 肯定要调度
    }
}
/*$PAGE*/
/*
...
#if 1
INT8U  OSTimeDlyHMSM (INT8U hours, INT8U minutes, INT8U seconds, INT16U milli)//输入的是绝对时间 会转换成tick
{
    INT32U ticks;
    INT16U loops;
    ...
    ticks = ((INT32U)hours * 3600L + (INT32U)minutes * 60L + (INT32U)seconds) * OS_TICKS_PER_SEC
          + OS_TICKS_PER_SEC * ((INT32U)milli + 500L / OS_TICKS_PER_SEC) / 1000L;//转换成tick    OS_TICKS_PER_SEC每秒钟有多少个tick
