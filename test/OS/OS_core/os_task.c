...
#if OS_TASK_CREATE_EN > 0//p_arg 是task的输入参数
...
INT8U  OSTaskDel (INT8U prio)
{
...
#endif
    OS_ENTER_CRITICAL();
    if (prio == OS_PRIO_SELF) {                                 /* See if requesting to delete self    *///0xff把自己删除 osstart用过
        prio = OSTCBCur->OSTCBPrio;                             /* Set priority to delete to current   *///OSTCBCur永远指向当前任务 全局变量 存储tcb指针 真实的id
    }
    ptcb = OSTCBPrioTbl[prio];//利用局部变量对prio操作 防止修改 current 
    if (ptcb == (OS_TCB *)0) {                                  /* Task to delete must exist           */
        OS_EXIT_CRITICAL();
        return (OS_TASK_DEL_ERR);
    }
    if (ptcb == (OS_TCB *)1) {                                  /* Must not be assigned to Mutex       *///Mutex互斥
        OS_EXIT_CRITICAL();
        return (OS_TASK_NOT_EXIST);
    }
	y            =  ptcb->OSTCBY;
	OSRdyTbl[y] &= ~ptcb->OSTCBBitX;
    if (OSRdyTbl[y] == 0x00) {                                  /* Make task not ready                 *///在就绪表中清0
        OSRdyGrp &= ~ptcb->OSTCBBitY;
    }
#if OS_EVENT_EN
    pevent = ptcb->OSTCBEventPtr;
...
#endif
    ptcb->OSTCBDly    = 0;                                      /* Prevent OSTimeTick() from updating  *///可以不用做
    ptcb->OSTCBStat   = OS_STAT_RDY;                            /* Prevent task from being resumed     */
    ptcb->OSTCBPendTO = FALSE;
	if (OSLockNesting < 255u) {                                 /* Make sure we don't context switch   *///
        OSLockNesting++;
	}
    OS_EXIT_CRITICAL();                                         /* Enabling INT. ignores next instruc. *///任务tcb之外的信息
    OS_Dummy();                                                 /* ... Dummy ensures that INTs will be *///不允许被任务打扰！ dummy虚拟的 这个函数是空的让 应用开发者去写释放资源 linux栈函数作用类似
    ...
    if (ptcb->OSTCBPrev == (OS_TCB *)0) {                       /* Remove from TCB chain               *///建议画图理解
        ptcb->OSTCBNext->OSTCBPrev = (OS_TCB *)0;//如果是头部 （不能是尾部，尾部是空闲任务）
        OSTCBList                  = ptcb->OSTCBNext;
    } else {
        ptcb->OSTCBPrev->OSTCBNext = ptcb->OSTCBNext;//正常流程
        ptcb->OSTCBNext->OSTCBPrev = ptcb->OSTCBPrev;
    }
    ptcb->OSTCBNext   = OSTCBFreeList;                          /* Return TCB to free TCB list         *///塞到头部
    OSTCBFreeList     = ptcb;//自己成为头部
#if OS_TASK_NAME_SIZE > 1
    ptcb->OSTCBTaskName[0] = '?';                               /* Unknown name                        */
    ptcb->OSTCBTaskName[1] = OS_ASCII_NUL;
#endif
    OS_EXIT_CRITICAL();
    OS_Sched();                                                 /* Find new highest priority task      *///或许有资源的释放 所以要进行调度
    return (OS_NO_ERR);
}
#endif
/*$PAGE*/
